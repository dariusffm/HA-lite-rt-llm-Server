...
> **Note**: the `log_level` enum is the intersection of bashio's
> typical levels and uvicorn's accepted levels (`critical|error|
> warning|info|debug|trace`). HA's `notice`/`fatal` are excluded so
> the value can be forwarded to `uvicorn --log-level` unmapped.
